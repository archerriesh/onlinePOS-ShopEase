<?php
session_start();
$pageCSS = '../../css/kelola-voucher.css';
        ...
        echo "<script>
            document.addEventListener('DOMContentLoaded', function() {
                Swal.fire({
                    ...
                    window.location.href='my-voucher.php'; // Diarahkan ke my-voucher
                });
            });
        </script>";
    }
...
if (isset($_POST['update'])) {
    ...
    mysqli_stmt_bind_param($stmtUpd, "sssssssss", $idPromo, $namaPromo, $tipePromo, $minimal, $pembayaran, $persen, $nominal, $start, $end);
...
if (!isset($_GET['id'])) {
    header("Location: my-voucher.php");
    exit;
}
...
$isNotOwner = ($data['idPenjual'] !== $idPenjualLogin);
...
if ($isNotOwner) {
    echo "<script>
        document.addEventListener('DOMContentLoaded', function() {
            Swal.fire({
                title: 'Akses Ditolak!',
                text: 'Seller tidak diperbolehkan mengelola promo milik Admin atau Seller lain.',
                icon: 'error',
                confirmButtonColor: '#ba704a',
                background: '#faf7f5',
                allowOutsideClick: false
            }).then(() => {
                window.location.href='promo-seller.php';
            });
        });
    </script>";
}
...
<div class="kelola-promo-page">
    ...
    <div class="promo-card">
        ...
        <form class="promo-form" method="POST" action="">
            ...
            <div class="promo-actions">
                <?php if (!$isNotOwner): ?>
                    <a href="#" class="btn-delete" style="text-decoration: none;" onclick="confirmDelete(event, 'kelola-voucher.php?id=<?= $data['idPromo'] ?>&action=delete')">Delete</a>
                    <button type="submit" name="update" class="btn-save">Save Changes</button>
                <?php else: ?>
                    <button type="button" class="btn-save" style="background-color: #ccc; cursor: not-allowed;" disabled>Restricted Voucher</button>
                <?php endif; ?>
            </div>
        </form>
    </div>
    ...
    <div class="back-wrapper">
        <a href="my-voucher.php" class="btn-back">← Back to Voucher List</a>
    </div>
</div>
